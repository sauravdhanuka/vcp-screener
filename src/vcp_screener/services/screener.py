    ...
    try:
        ...
        for symbol, df in price_cache.items():
            ...
            candidates.append({
                ...
                "rs_percentile": float(round(rs_pct, 1)),
                "vcp_score": float(round(vcp_sc, 1)),
                "pivot_price": float(vcp_result.get("pivot_price")) if vcp_result.get("pivot_price") is not None else None,
                "base_depth_pct": float(round(vcp_result.get("base_depth_pct", 0), 1)),
                "num_contractions": int(vcp_result.get("num_contractions", 0)),
                "tightness_ratio": float(round(vcp_result.get("tightness_ratio", 0), 2)),
                "volume_dry_up": float(round(vcp_result.get("volume_dry_up_pct", 0), 1)),
                "base_duration_days": int(vcp_result.get("base_duration_days", 0)),
                "trend_conditions": trend_result["conditions"],
                "contractions": vcp_result.get("contractions", []),
                "is_monotonic": bool(vcp_result.get("is_monotonic", False)),
                "shakeout_detected": bool(vcp_result.get("shakeout_detected", False)),
                "volume_quietness": float(vcp_result.get("volume_quietness", 1.0)) if vcp_result.get("volume_quietness") is not None else None,
