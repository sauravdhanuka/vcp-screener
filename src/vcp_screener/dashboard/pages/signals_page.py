...
@st.cache_data(ttl=300, show_spinner="Loading VCP signals...")
...
def render():
    ...
    with col_refresh:
        if st.button("Refresh", use_container_width=True):
            _load_vcp_signals.clear()
            _load_mr_signals.clear()
            st.rerun()

    view = st.radio("", ["VCP Signals", "MR Signals"], horizontal=True, label_visibility="collapsed")

    if view == "VCP Signals":
        signals = _load_vcp_signals()
        if not signals:
