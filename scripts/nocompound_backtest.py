"""10Y backtest WITHOUT compounding for Winner and Runner-up."""
...
for cfg_name, mr_cfg in CONFIGS:
    engine = BacktestEngine(
        ...
        mr_config=mr_cfg, compounding=False,
    )
    ...
    print(f"{'='*110}")
    print(f"  {cfg_name}  [NO COMPOUNDING]")
    ...
    print(f"  Win Rate: {r['win_rate_pct']:.1f}%  |  Avg Gain: {r['avg_gain_pct']:.1f}%  |  Avg Loss: {r['avg_loss_pct']:.1f}%  |  Avg Hold: {r['avg_hold_days']:.0f}d")
    print(f"{'='*110}")
    ...
    for reason, stats in sorted(exit_stats.items(), key=lambda x: -x[1]["count"]):
        ...
        print(f"    {reason:<25} {stats['count']:>4} trades  WR:{wr:>5.1f}%  PnL: Rs {stats['pnl']:>+12,.0f}")
